...
test("status command refreshes quota, persists store, and ends with controlled interrupt", async () => {
  ...
  assert.equal(messageLines[0] ?? "", "[default] premium 10/50 | [claude-3.7] premium 10/50 | [gpt-4.1] premium 10/50")
  assert.doesNotMatch(messageLines[0] ?? "", /current active/i)
  assert.doesNotMatch(messageLines[0] ?? "", /\bchat\b/i)
  assert.doesNotMatch(messageLines[0] ?? "", /\bcompletions\b/i)
  assert.equal(messageLines[1] ?? "", "活跃组: alice, bob")
  assert.equal(messageLines[2] ?? "", "路由组: claude-3.7 -> alice; gpt-4.1 -> bob, alice")

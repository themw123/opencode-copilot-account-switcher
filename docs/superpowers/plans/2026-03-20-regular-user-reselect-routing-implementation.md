...
- [ ] **Step 4: 同步在 `src/routing-state.ts` 扩展 `RouteDecisionEvent["reason"]`**

最小实现要求：

- 把 `unbound-fallback` 加入 reason 联合类型
- 保证 Task 1 结束时类型系统已经接受这个新 reason

- [ ] **Step 5: 在发送前对 `unbound-fallback` 去掉 outbound `x-initiator: agent`**
...
- [ ] **Step 4: 在 `src/plugin-hooks.ts` 补齐兼容实现并确认 `rate-limit-switch` 不回退**

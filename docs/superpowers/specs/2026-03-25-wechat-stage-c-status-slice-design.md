...
> 现状补充（不改变阶段 C 边界）：当前仓库已在阶段 C 基座上继续完成后续通知发送链路与 handle 驱动 `/reply` / `/allow` 闭环；本文件中的“非目标”仅用于说明阶段 C 当时不提前实现的范围。
...
3. 等待聚合窗口（实现默认 `5_000ms`，并可通过环境变量覆盖；测试仅验证可收紧到 `1500ms`）；
4. 收集 bridge 回包；
...
10. 本阶段仍未实现通知、token stale fallback、`/reply`、`/allow`（阶段 C 当时口径）。
